...
function choice_check() {
    ...
    if ($readStatus -NotMatch "^[qQ]$|1|2|3|4|5") {
...
function choice_remove_more_more() {
   ...
   ← [B]ack
    ...
    # Call the next function
    choice_check_remove_more -readChoiceRemoveMoreMore $readChoiceRemoveMoreMore
}
...
$destination = "$workdir\chrome.exe" # Where to download to
Invoke-WebRequest $source -OutFile $destination # Download from source to destination
